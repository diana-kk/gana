...
topArea.text(22, 107, 'カ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'ク').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_kurasu.use().transform('t140, 385').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(160, 520, ['ク', 'ラス', '반/학급']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
