...
// 행의 해당 문자
topArea.circle(30, 250, 20).attr({
  'fill': '#ececec',
  'stroke-width': 2
});
topArea.text(22, 257, 'な').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'ぬ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_nuno.use().transform('t135 390').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 520, ['ぬ', 'の', '천']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
