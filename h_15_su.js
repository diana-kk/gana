...
// 행의 해당 문자
topArea.circle(30, 150, 20).attr({
  'fill': '#ececec',
  'stroke-width': 2
});
topArea.text(22, 157, 'さ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'す').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_sushi.use().transform('t135 390').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 520, ['す', 'し', '초밥']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
