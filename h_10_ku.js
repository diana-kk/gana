...
// 행의 해당 문자
topArea.circle(30, 100, 20).attr({
  'fill': '#ececec',
  'stroke-width': 2
});
topArea.text(22, 107, 'か').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'く').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_kuma.use().transform('t140, 385').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 520, ['く', 'ま', '곰']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
