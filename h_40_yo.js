...
topArea.text(22, 407, 'や').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'よ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_yoru.use().transform('t130 390').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 520, ['よ', 'る', '밤']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
