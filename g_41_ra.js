...
//이미지
var g_razio = Paper.image('img/g_razio.jpg', 0, 0, 115, 100).toDefs();
...
// 행의 해당 문자
topArea.circle(30, 450, 20).attr({
  'fill': '#ececec',
  'stroke-width': 2
});
topArea.text(22, 457, 'ラ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'ラ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_razio.use().transform('t130 380').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(170, 520, ['ラ', 'ジオ', '라디오']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
