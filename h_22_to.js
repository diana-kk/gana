...
// 행의 해당 문자
topArea.circle(30, 200, 20).attr({
  'fill': '#ececec',
  'stroke-width': 2
});
topArea.text(22, 207, 'た').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
// 단의 해당 문자
bottomArea.circle(315, 50, 20).attr({
  'fill': '#FCF5D4',
  'stroke': '#A1D8C8',
  'stroke-width': 3
});
bottomArea.text(307, 57, 'と').attr({
  'font-size': 20,
  'font-weight': 'bold'
});
...
bottomArea.text(171, 240, 'と').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_tokei.use().transform('t135 385').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 520, ['と', 'けい', '시계']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
