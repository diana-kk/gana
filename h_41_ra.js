...
topArea.text(22, 457, 'ら').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'ら').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_rakuda.use().transform('t130 390').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 520, ['ら', 'くだ', '낙타']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
