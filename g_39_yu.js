...
// 행의 해당 문자
topArea.circle(30, 400, 20).attr({
  'fill': '#ececec',
  'stroke-width': 2
});
topArea.text(22, 407, 'ヤ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
for (var i = 0; i < t11.length; i++) {
  bottomArea.circle(156 + (i * 53), 50, 20).attr({
    'fill': '#FCF5D4'
  });
  bottomArea.text(148 + (i * 53), 57, t11[i]).attr({
    'font-size': 20,
    'fill': 'gray'
  });
}
...
// 단의 해당 문자
bottomArea.circle(209, 50, 20).attr({
  'fill': '#FCF5D4',
  'stroke': '#FFB2AF',
  'stroke-width': 3
});
bottomArea.text(201, 57, 'ユ').attr({
  'font-size': 20,
  'font-weight': 'bold'
});
...
bottomArea.text(171, 240, 'ユ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_yunihoomu.use().transform('t140 380').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(132, 520, ['ユ', 'ニホーム', '유니폼']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
