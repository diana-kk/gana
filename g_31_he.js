...
topArea.text(22, 307, 'ハ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'ヘ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_heapinn.use().transform('t135 390').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(150, 520, ['ヘ', 'アピン', '머리핀']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
