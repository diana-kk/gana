...
// 단의 해당 문자
bottomArea.circle(185, 50, 20).attr({
  'fill': '#FCF5D4',
  'stroke': '#FFB2AF',
  'stroke-width': 3
});
bottomArea.text(177, 57, 'ワ').attr({
  'font-size': 20,
  'font-weight': 'bold'
});
...
bottomArea.text(171, 240, 'ワ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_wannpiisu.use().transform('t140 385').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(130, 520, ['ワ', 'ンピース', '원피스']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
