...
topArea.text(22, 207, 'タ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'ト').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_tomato.use().transform('t135 385').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 520, ['ト', 'マト', '토마토']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
