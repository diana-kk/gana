...
// 단의 해당 문자
bottomArea.circle(105, 50, 20).attr({
  'fill': '#FCF5D4',
  'stroke': '#A1D8C8',
  'stroke-width': 3
});
bottomArea.text(97, 57, 'あ').attr({
  'font-size': 20,
  'font-weight': 'bold'
});
...
bottomArea.text(171, 240, 'あ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_aka.use().transform('t130, 400').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 520, ['あ', 'か', '빨강']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
