...
topArea.text(22, 157, 'サ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'サ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_saakasu.use().transform('t135 385').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(145, 520, ['サ', 'ーカス', '서커스']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
