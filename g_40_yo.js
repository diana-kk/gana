...
// 단의 해당 문자
bottomArea.circle(262, 50, 20).attr({
  'fill': '#FCF5D4',
  'stroke': '#FFB2AF',
  'stroke-width': 3
});
bottomArea.text(254, 57, 'ヨ').attr({
  'font-size': 20,
  'font-weight': 'bold'
});
...
bottomArea.text(171, 240, 'ヨ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_yooyoo.use().transform('t130 390').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(160, 520, ['ヨ', 'ーヨー', '요요']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
