...
topArea.text(22, 257, 'ナ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 240, 'ヌ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
g_nuudoru.use().transform('t135 390').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(160, 520, ['ヌ', 'ードル', '누들']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
